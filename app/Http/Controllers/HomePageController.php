...
class HomePageController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($request->banner_type == 'slider') {
            if ($request->hasFile('slider_images')) {
                ...
                foreach ($request->file('slider_images') as $image) {
                    $sliderImages[] = $image->store('homepage/slider');
                }
        ...
        if ($request->hasFile('world_of_kerovit_image')) {
            $homePage->world_of_kerovit_image = $request->file('world_of_kerovit_image')->store('homepage/world_of_kerovit');
        }

        if ($request->hasFile('catalogue_pdf_1')) {
            $homePage->catalogue_pdf_1 = $request->file('catalogue_pdf_1')->store('homepage/catalogues');
        }
        if ($request->hasFile('catalogue_pdf_2')) {
            $homePage->catalogue_pdf_2 = $request->file('catalogue_pdf_2')->store('homepage/catalogues');
        }
